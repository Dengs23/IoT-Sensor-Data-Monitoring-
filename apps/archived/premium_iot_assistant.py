    ...
    with tab1:
        ...
        with col1:
            if st.button("ROI Analysis", use_container_width=True):
                st.session_state.chat_input = "What's the ROI for IoT in commercial buildings?"
        
        with col2:
            if st.button("Sensor Advice", use_container_width=True):
                st.session_state.chat_input = "Best sensors for energy monitoring?"
        
        with col3:
            if st.button("Network Help", use_container_width=True):
                st.session_state.chat_input = "Zigbee vs WiFi for IoT?"
